...
end_time="2015-03-11T00:00"
unit="hour"

input_file_name=$1
output_file_name="series_`echo $input_file_name | sed 's/\..*$//' | awk -F'/' '{print $NF}'`"
output_dir=counts

rm -f counts/*
while read topic; do
    echo "Doing $topic"
    search_api.py -f "\"$topic\"" -s $start_time -e $end_time -b $unit timeline | jq -c "{\"${topic}\":"' [.results[] | .count]}' > ${output_file_name}.json
    sleep 0.3
